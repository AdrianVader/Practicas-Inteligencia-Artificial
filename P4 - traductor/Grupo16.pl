...
%	Traduce tanto del inglés al español como del español al inglés, por lo que también podemos introducir:
%	frase(Trad, [the,beautiful,girls,drink,white,milkshakes],[]).
%	Para ejecutar el bucle escribimos:
%	traductor.
%	[los,niños,beben].	<- Aquí cualquier frase
%	[].					<- Lista vacía para terminar
%	Al terminar se imprimirán los nombres (palabras clave) de todas las frases solo si todas están escritas en inglés
...
% Hechos que representan el diccionario completo en español, contiene todos los verbos,
% terminaciones, nombres, morfemas, adjetivos, artículos y preposiciones, junto con
...
% Hechos que representan el diccionario completo en inglés, contiene todos los verbos,
% terminaciones, nombres, morfemas, adjetivos, artículos y preposiciones, junto con
% toda la información adicional necesaria.
...
terminacion_ing(singular, s). % Solo existe una terminación, para la 3ª persona del singular.
...
% Hechos que representan la traducción de las raíces de las palabras inglesas y españolas.
% En algunos casos (como es o son) la traducción es directa.
...
% Reglas que traducen palabras enteras de español a inglés (la raíz, que traducimos
% automáticamente con los hechos, y un morfema o terminación si es necesario). Guardamos
% información como el número (singular, plural), el género (masculino, femenino) y el tipo
% (regular, copulativo, intransitivo) para después controlar la concordancia entre todas las
...
% Reglas que traducen palabras enteras de inglés a español (la raíz, que traducimos
% automáticamente con los hechos, y un morfema o terminación si es necesario). Guardamos
% información como el número (singular, plural), el género (masculino, femenino) y el tipo
% (regular, copulativo, intransitivo) para después controlar la concordancia entre todas las
% palabras de una frase.
...
% Reglas que supervisan la formación de grupos nominales y verbales en español, para luego traducirlos
% a inglés. Se controla que las palabras estén en el orden adecuado (teniendo en cuenta que el orden no es
% el mismo en español y en inglés) y que no se construyen frases incorrectas.
...
% Verbo + preposición + grupo nominal
...
% Verbo + preposición + nombre + adjetivo
...
% Reglas que supervisan la formación de grupos nominales y verbales en inglés, para luego traducirlos
% a español. Se controla que las palabras estén en el orden adecuado (teniendo en cuenta que el orden no es
% el mismo en español y en inglés) y que no se construyen frases incorrectas.
% Consideramos distintos casos particulares en los que los verbos tengan que ser de cierto tipo.
...
% una frase vacía) nos devuelve todos los nombres usados en todas las frases.
% Lo hemos implementado de manera que solo funcione la parte opcional si todas las frases han sido escritas en inglés.
